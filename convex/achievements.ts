...
export const checkAndUnlockAchievements = internalAction({
	...
	> => {
		...
		const allAchievements: any = await ctx.runQuery(
			api.queries.getAllAchievements,
			{},
		);

		const userAchievements: any = await ctx.runQuery(
			internal.queries.internalGetUserAchievements,
			{ userId },
		);
		const unlockedAchievementIds = new Set(
			userAchievements.map((ua: any) => ua.id),
		);
...
export const recalculateUserAchievements = internalAction({
	...
	handler: async (ctx, args) => {
		...
		const existingAchievements = await ctx.runQuery(
			internal.queries.internalGetUserAchievements,
			{ userId },
		);
		...
		for (const achievement of achievements) {
			...
			switch (achievement.conditionType) {
				...
				case "streak":
					shouldUnlock =
						Math.max(userStats.currentStreak, userStats.longestStreak) >=
				...
				case "speed_completion":
					shouldUnlock = levelProgress.some(
						(p: any) =>
							p.isCompleted &&
							p.timeSpent &&
							p.timeSpent <= achievement.conditionValue,
					);
					break;
			}

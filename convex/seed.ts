...
export const seedApps = action({
	args: {},
	handler: async (ctx) => {
		const existingPromptPal = await ctx.runQuery(api.apps.getApp, {
			appId: "prompt-pal",
		});
...
export const seedLearningModules = action({
	...
	handler: async (ctx) => {
		const modules = [
			{
				...
				description:
					"Analyze target images and write prompts to recreate them using Gemini AI. Master the art of visual description and style transfer.",
				objectives: [
			...
			},
			{
				...
				description:
					"Transform programming requirements into functional code. Execute your generated code in a sandbox and verify correctness through automated testing.",
				objectives: [
			...
			},
			{
				...
				description:
					"Master marketing briefs and generate persuasive copy. Analyze tone, style, and effectiveness against real-world brief requirements.",
				objectives: [
		...
		for (let i = 0; i < modules.length; i++) {
			...
			const existing = await ctx.runQuery(api.queries.getLearningModuleById, {
				id: module.id,
			});
			if (!existing) {
		...
		const resources = [
			{
				...
				description:
					"Use practical lighting language to control mood, depth, and realism in image prompts.",
				category: "IMAGE GENERATION",
			...
			},
			{
				...
				description:
					"A fast lookup of style and quality terms that consistently improve image outputs.",
				category: "IMAGE GENERATION",
				difficulty: "beginner" as const,
				estimatedTime: 6,
				...
				content: {
					terms: [
						...
						},
						{
							term: "Photorealistic",
							definition:
								"Pushes materials, lighting, and detail toward realism.",
							example:
								"photorealistic product shot, natural reflections, studio backdrop",
						},
						{
							term: "Concept Art",
							definition:
								"Leans toward exploratory design language and stylized detail.",
							example:
								"sci-fi concept art, atmospheric perspective, dramatic silhouette",
						},
					],
				},
			},
			{
				...
				description:
					"Reference snippets for choosing the right iteration pattern in coding challenges.",
				category: "CODING & LOGIC",
				difficulty: "beginner" as const,
				estimatedTime: 5,
					...
					snippets: [
						{
							...
							description:
								"Use when every input item should produce one output item.",
						},
						{
							title: "Filter then map",
							code: "const activeEmails = users.filter((u) => u.active).map((u) => u.email);",
							description:
								"Split selection and transformation for readable prompts and code.",
						},
						{
			...
			},
			{
				...
				description:
					"A repeatable workflow to catch errors, tighten requirements, and iterate quickly.",
				category: "CODING & LOGIC",
			...
			},
			{
				...
				description:
					"Prompt templates for AIDA, PAS, and feature-to-benefit conversion.",
				category: "COPYWRITING",
				difficulty: "beginner" as const,
				estimatedTime: 6,
			...
			},
			{
				...
				description:
					"How better prompt constraints improved a campaign CTA click-through rate.",
				category: "COPYWRITING",
				difficulty: "intermediate" as const,
				estimatedTime: 7,
				...
				content: {
					challenge:
						"Open rates were healthy, but CTA clicks underperformed because copy was generic and value was unclear.",
					solution:
						"The team used a stricter prompt template: audience pain points, tone constraints, one concrete proof point, and three CTA options ranked by urgency.",
					result:
						"CTA click-through increased by 28% over two campaign cycles.",
				},
			},
			{
				...
				description:
					"Get stronger outputs by explicitly asking the model to critique and improve its first draft.",
				category: "GENERAL",
				difficulty: "beginner" as const,
				estimatedTime: 4,
					...
					sections: [
						{
							...
							content:
								"Ask for an initial answer, a short self-critique against your constraints, and a revised final version.",
							example:
								"Generate a first draft. Then list 3 weaknesses against my goal. Rewrite with those fixed.",
						},
						{
							title: "Keep Constraints Stable",
							content:
								"If the model drifts, restate non-negotiables like format, audience, tone, and length before each rewrite.",
							example:
								"Keep B2B tone, max 120 words, and include one quantified claim.",
						},
					],
				},
			},
		...
		for (let i = 0; i < resources.length; i++) {
			...
			if (existing) {
				await ctx.runMutation(
					internal.mutations.updateLearningResource,
					payload,
				);
				logSeed(`Updated learning resource: ${res.title}`);
			} else {
				await ctx.runMutation(
					internal.mutations.createLearningResource,
					payload,
				);
...
export const seedLevels = action({
	...
	handler: async (ctx) => {
		...
		const duplicatesRemoved = await ctx.runMutation(
			internal.mutations.deleteDuplicateLevels,
			{
				appId: "prompt-pal",
			},
		);
		...
		const staleRemoved = await ctx.runMutation(
			internal.mutations.deleteLevelsNotInSet,
			{
				appId: "prompt-pal",
...
export const seedAchievements = action({
	...
	handler: async (ctx) => {
		...
		for (const achievement of achievements) {
			const existing = await ctx.runQuery(api.queries.getAchievementById, {
				id: achievement.id,
			});
			if (!existing) {

...
export const deleteCurrentUserData = mutation({
	...
	handler: async (ctx) => {
		...
		deleted.userStatistics = await deleteDocumentsByIndex(
			ctx,
			"userStatistics",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.userPreferences = await deleteDocumentsByIndex(
			ctx,
			"userPreferences",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.userLevelAttempts = await deleteDocumentsByIndex(
			ctx,
			"userLevelAttempts",
			"by_user_created",
			(q) => q.eq("userId", userId),
		);
		deleted.userProgress = await deleteDocumentsByIndex(
			ctx,
			"userProgress",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.gameSessions = await deleteDocumentsByIndex(
			ctx,
			"gameSessions",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.gameProgress = await deleteDocumentsByIndex(
			ctx,
			"gameProgress",
			"by_user_app",
			(q) => q.eq("userId", userId),
		);
		deleted.userModuleProgress = await deleteDocumentsByIndex(
			ctx,
			"userModuleProgress",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.userModules = await deleteDocumentsByIndex(
			ctx,
			"userModules",
			"by_user_app",
			(q) => q.eq("userId", userId),
		);
		deleted.userQuestCompletions = await deleteDocumentsByIndex(
			ctx,
			"userQuestCompletions",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.userDailyQuests = await deleteDocumentsByIndex(
			ctx,
			"userDailyQuests",
			"by_user_date",
			(q) => q.eq("userId", userId),
		);
		deleted.userQuests = await deleteDocumentsByIndex(
			ctx,
			"userQuests",
			"by_user_app",
			(q) => q.eq("userId", userId),
		);
		deleted.userAchievements = await deleteDocumentsByIndex(
			ctx,
			"userAchievements",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.aiGenerations = await deleteDocumentsByIndex(
			ctx,
			"aiGenerations",
			"by_user_created",
			(q) => q.eq("userId", userId),
		);
		deleted.userEvents = await deleteDocumentsByIndex(
			ctx,
			"userEvents",
			"by_user_app",
			(q) => q.eq("userId", userId),
		);
		deleted.errorLogs = await deleteDocumentsByIndex(
			ctx,
			"errorLogs",
			"by_user",
			(q) => q.eq("userId", userId),
		);
		deleted.appPlans = await deleteDocumentsByIndex(
			ctx,
			"appPlans",
			"by_user_app",
			(q) => q.eq("userId", userId),
		...
		return {
			...
			userId,
			deleted,
			deletedTotal: Object.values(deleted).reduce(
				(sum, value) => sum + value,
				0,
			),
		};
		...
		if (imageUrl) {
			try {
				...
				if (
					...
					error.message.includes("trusted domain")
				) {
					throw error;
				}
...
async function completeQuestForUser(
	...
	const existing = await ctx.db
		...
		.withIndex("by_user_quest", (q) =>
			q.eq("userId", userId).eq("questId", questId),
		)
		.first();
	...
	handler: async (ctx, args) => {
		...
		const levels = args.appId
			? await ctx.db
					...
					.withIndex("by_app_order", (q) => q.eq("appId", args.appId!))
					.collect()
			: await ctx.db.query("levels").collect();

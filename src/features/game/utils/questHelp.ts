...
export function buildQuestHelpContent(
	level: Level,
	visibleHints: string[],
): QuestHelpContent {
	...
	if (level.type === "code") {
		return {
			...
			headline,
			summary:
				"Use this panel for the success criteria without scrolling back to the instruction card.",
			...
			sections: [
				{
					...
					items: uniqueItems([
						level.description ||
							"Guide the model to return working JavaScript for this challenge.",
					]),
				},
				{
					...
					items: uniqueItems([
						level.functionName
							? `Ask for a function named ${level.functionName}.`
							: undefined,
						level.language
							? `The response must be valid ${level.language}.`
	...
	if (level.type === "copywriting") {
		return {
			...
			headline,
			summary:
				"Keep the brief requirements in view while you shape the prompt strategy.",
			...
			sections: [
				{
					...
					items: uniqueItems([
						level.description ||
							"Write a prompt that leads to stronger conversion-focused copy.",
					]),
				},
				{
	...
	return {
		...
		headline,
		summary:
			"Use the target image as your reference and describe the shot with clear, concrete visual detail.",
		...
		sections: [
			{
				...
				items: uniqueItems([
					level.description ||
						"Write a prompt that recreates the target image as closely as possible.",
				]),
			},
			{
				...
				items: uniqueItems([
					...
					level.style
						? `Keep the visual direction aligned with the ${level.style} style.`
						: undefined,

...
export interface PromptInputViewProps {
	...
	onInputFocus?: () => void;
	onInputSelectionChange?: (selection: { start: number; end: number }) => void;
}
...
export function PromptInputView({
	...
	const { spendXP, xp } = useUserProgressStore();
	...
	const handleGetHint = useCallback(async () => {
		...
		try {
			const hint = await NanoAssistant.getHint(
				value,
				moduleType,
				level as unknown as StoreLevel,
			);
	...
	}, [hintCooldown, isLoadingHint, level, moduleType, spendXP, value, xp]);
								...
								{noHintsLeft
									...
									: hintCooldown > 0
										...
										: hintsUsed === 0
											...
											: `Hint (${HINT_XP_COST} XP • ${hintsRemaining}/${maxHints})`}
							</Text>
						</>
					)}
				</TouchableOpacity>
			...
			<Card className="p-6 rounded-[32px] border-2 border-primary/30 bg-surfaceVariant/20">
				...
				<Input
					...
					onSelectionChange={(event) =>
						onInputSelectionChange?.(event.nativeEvent.selection)
					}
				/>

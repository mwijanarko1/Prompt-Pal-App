...
export function TargetImageView({
	...
	if (!hasValidSource) {
		return (
			<View
				className={`relative overflow-hidden aspect-square items-center justify-center bg-surfaceVariant ${className}`}
			>
	...
	const imageSource =
		typeof source === "object" && "uri" in source ? source : source;

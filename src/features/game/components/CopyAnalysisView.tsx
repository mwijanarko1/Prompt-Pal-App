...
export function CopyAnalysisView({
	...
	if (isLoading) {
		return (
			<Card className="p-6 rounded-[32px]" variant="elevated">
				<Text className="text-onSurface text-center">
					Analyzing your copy...
				</Text>
			</Card>
		);
	...
	const missedRequirements =
		requirements?.filter(
			(req) => !copy.toLowerCase().includes(req.toLowerCase()),
		) ?? [];
				...
				<Card className="p-6 rounded-[32px] mb-6" variant="elevated">
					<View className="flex-row items-center justify-between mb-4">
						...
						<View className="flex-row items-center">
							<Text className="text-primary text-3xl font-black mr-2">
								{score}%
							</Text>
							<View
						...
						<View className="flex-row flex-wrap justify-center">
							{metrics.map((metric, index) => {
								...
								return (
									<View
										key={index}
										className="items-center mx-3 mb-4 min-w-[70px]"
									>
										<View

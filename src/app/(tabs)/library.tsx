...
const ResourceCard = memo(function ResourceCard({
	...
	return (
		<Pressable onPress={handlePress} className="mb-4 active:opacity-70">
			<View className="flex-row items-center p-4 rounded-[24px] border-0 bg-surfaceVariant/30">
				...
				<View className="flex-1">
					...
					<Text className="text-onSurfaceVariant text-xs" numberOfLines={1}>
	...
	return (
		<View className="mb-10">
			<View className="flex-row justify-between items-end mb-4">
				<View>
					<Text className="text-primary text-[10px] font-black uppercase tracking-[2px] mb-1">
						Category
					</Text>
			...
			{category.resources.length > 0 && (
				<View>
					<Text className="text-onSurfaceVariant text-[10px] font-black uppercase tracking-widest mb-3">
						Resources
					</Text>
...
const UserSummary = memo(function UserSummary({
	userSummary,
}: UserSummaryProps) {
	return (
		...
		>
			...
			<View className="px-6 pt-4 pb-2">
				<View className="flex-row justify-between items-center mb-6">
					<View>
						<Text className="text-primary text-[10px] font-black uppercase tracking-[4px] mb-1">
							Academy
						</Text>
						<Text className="text-onSurface text-3xl font-black">
							Knowledge Base
						</Text>
					</View>
			...
			>
				...
				{libraryData?.categories
					...
					.map((cat: Category, idx: number) => (
						<CategorySection key={idx} category={cat} index={idx} />
					))}
			</ScrollView>
		</SafeAreaView>
	);

...
export const StatCard = memo(function StatCard({
		...
		return (
			<View className="bg-surface/50 border border-outline/30 p-4 rounded-3xl flex-1 mx-1 items-center">
				...
				<View className="flex-row items-center">
					<Ionicons
						name={getIconName(icon || "stats-chart")}
						size={14}
						color={onSurfaceVariant}
					/>
					<Text className="text-onSurfaceVariant text-[8px] font-black uppercase ml-1 tracking-widest">
						{label}
					</Text>
				</View>
			</View>
	...
	if (variant === "featured") {
		return (
			...
			>
				<Text className="text-onSurfaceVariant text-[10px] font-black uppercase tracking-widest mb-2 opacity-60">
					{label}
				</Text>
				...
				{trend && (
					<View className="flex-row items-center">
						<Ionicons
							...
							color={color}
						/>
						<Text className="text-xs font-bold ml-1" style={{ color }}>
							{trend}
						</Text>
					</View>
				)}

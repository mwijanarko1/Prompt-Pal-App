...
export function renderResourceContent(resource: Resource) {
	...
	if (!content)
		return (
			<Text className="text-onSurfaceVariant italic">
	...
	switch (type) {
		case "lexicon": {
			const terms =
				asArray(contentData.terms).length > 0
				...
				<View className="space-y-4">
					{terms.map((item, index: number) => {
						...
						return (
							<View
								key={index}
								className="bg-surfaceVariant/20 p-4 rounded-2xl border border-outline/10"
							>
								<Text className="text-primary font-black text-lg mb-1">
									{term || `Term ${index + 1}`}
								</Text>
								<Text className="text-onSurface text-sm leading-5">
									{definition || "Definition unavailable."}
								</Text>
								{example ? (
									<View className="mt-3 bg-black/10 p-2 rounded-lg">
										<Text className="text-onSurfaceVariant text-xs italic">
											Example: {example}
										</Text>
									</View>
								) : null}
		...
		case "cheatsheet": {
			const snippets =
				asArray(contentData.snippets).length > 0
					...
					{snippets.map((item, index: number) => {
						...
						return (
							<View key={index} className="space-y-2">
								<Text className="text-onSurface font-bold text-base">
									{title}
								</Text>
		...
		case "guide": {
			...
			if (sections.length === 0) {
				return (
					<Text className="text-onSurfaceVariant italic">
						No guide sections available yet.
					</Text>
				);
			}
							...
							<View key={index} className="space-y-3">
								...
								{tips ? (
									<View className="bg-primary/10 p-4 rounded-2xl border border-primary/20 mt-4 mb-4">
										...
										<Text className="text-onSurface text-xs italic leading-5">
											{tips}
										</Text>
									</View>
								) : null}
			...
			return (
				<View className="space-y-6">
					<View className="bg-surfaceVariant/30 p-5 rounded-[32px] border border-outline/10">
						<Text className="text-primary font-black text-xs uppercase tracking-[2px] mb-2">
							The Challenge
						</Text>
					...
					<View className="space-y-4">
						...
						<Text className="text-onSurfaceVariant text-sm leading-6">
							{asString(contentData.solution, "Solution details unavailable.")}
						</Text>
					</View>

					<View className="bg-success/10 p-5 rounded-[32px] border border-success/20">
						<Text className="text-success font-black text-xs uppercase tracking-[2px] mb-2">
							Key Result
						</Text>
						<Text className="text-onSurface text-base leading-6 font-bold">
							{asString(contentData.result, "Result details unavailable.")}
						</Text>
					</View>
				</View>
			);
		...
		case "prompting-tip": {
			...
			if (sections.length === 0) {
				return (
					<Text className="text-onSurfaceVariant italic">
						No prompting tips available yet.
					</Text>
				);
			}
			...
			return (
				<View className="space-y-6">
					{sections.map((section, index: number) => {
						...
						const body = asString(
							row.content,
							asString(row.body, asString(row.text)),
						);
						const example = asString(row.example);
						...
						return (
							<View key={index} className="space-y-3">
								<View className="flex-row items-center">
									...
									<Text className="text-onSurface font-black text-lg ml-2">
										{title}
									</Text>
								</View>
								...
								{example ? (
									<View className="bg-primary/5 p-4 rounded-2xl border border-primary/10 ml-7">
										...
										<Text className="text-onSurface text-sm italic leading-5">
											{example}
										</Text>
									</View>
								) : null}
				...
				>
					...
					<View className="px-6 pt-4 pb-4 border-b border-outline/5">
						...
						<View className="flex-row items-center">
							...
							<Text className="text-onSurfaceVariant text-xs ml-1 mr-4">
								{resource.estimatedTime || 5} min read
							</Text>
							<Ionicons
					...
					<View className="px-6 pt-4 pb-8 border-t border-outline/5 bg-surface">
						<Button onPress={onClose} variant="primary" className="w-full">
							Got it
						</Button>
					</View>
				</View>

...
export class CodeScoringService {
	...
	static async scoreCode(input: CodeScoringInput): Promise<CodeScoringResult> {
		...
		try {
			...
			const languageMismatch = this.detectLanguageMismatch(
				trimmedCode,
				normalizedLanguage,
			);
			if (languageMismatch) {
			...
			logger.error("CodeScoringService", error, {
				operation: "scoreCode",
				input,
			});
	...
	private static async runTestCases(
		...
		return Promise.all(
			testCases.map((testCase) => this.runSingleTest(prepared, testCase)),
		);
	}
	...
	private static async runSingleTest(
		...
		try {
			...
			return {
				...
				error: passed
					? undefined
					: this.createMismatchError(actualOutput, testCase.expectedOutput),
			};
		} catch (error) {
			logger.warn("CodeScoringService", "Test execution failed", {
	...
	private static normalizePrepareError(error: unknown): {
		message: string;
		syntaxValid: boolean;
	} {
	...
	private static detectLanguageMismatch(
		code: string,
		language: "javascript",
	): string | null {
		...
		if (expectedName) {
			...
			if (!foundName) {
				...
				const looksLikeExpression =
					/^(\(|function\b|async\b|[A-Za-z_$][\w$]*\s*=>)/.test(trimmed);
	...
	private static generateFeedback(
		...
		if (failedTests.length > 0) {
			...
			const timeoutFailures = failedTests.filter((test) =>
				test.error?.includes("timed out"),
			);
	 ...
	 */
	static async scoreCodes(
		inputs: CodeScoringInput[],
	): Promise<CodeScoringResult[]> {
		...
		return results.map((result, index) => {
			...
			logger.error("CodeScoringService", result.reason, {
				operation: "scoreCodes",
				index,
			});
			return {
				score: 0,

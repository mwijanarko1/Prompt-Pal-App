...
export class CopyScoringService {
	...
	static async scoreCopy(input: CopyScoringInput): Promise<CopyScoringResult> {
		...
		try {
			...
			const overallScore = calculateCopyOverallScore(
				metrics,
				elementChecks,
				withinLimit,
			);
			...
			logger.error("CopyScoringService", error, {
				operation: "scoreCopy",
				input,
			});
	...
	private static async analyzeMetrics(
		...
		try {
			...
			logger.warn("CopyScoringService", "AI analysis failed, using fallback", {
				error,
			});
	 ...
	 */
	static async scoreCopies(
		inputs: CopyScoringInput[],
	): Promise<CopyScoringResult[]> {
		...
		return results.map((result, index) => {
			...
			logger.error("CopyScoringService", result.reason, {
				operation: "scoreCopies",
				index,
			});
			return {
				score: 0,

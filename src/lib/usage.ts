...
export class UsageClient {
	...
	static async getUsage(): Promise<UsageStats> {
		...
		logger.warn(
			"UsageClient",
			"getUsage() is deprecated. Use the useUsage hook instead.",
		);
		throw new Error(
			"UsageClient.getUsage() is deprecated. Use the useUsage hook in React components.",
		);
	}
	 ...
	 */
	static getRemainingCalls(usage: UsageStats): {
		textCalls: number;
		imageCalls: number;
	} {
		return {
	...
	static isNearLimit(
		usage: UsageStats,
		thresholdPercent: number = DEFAULT_USAGE_THRESHOLD,
	): boolean {
		...
		return (
			textUsagePercent >= thresholdPercent ||
			imageUsagePercent >= thresholdPercent
		);
	}
	...
	static getUsagePercentage(usage: UsageStats, type: "text" | "image"): number {
		...
		const limit =
			type === "text" ? usage.limits.textCalls : usage.limits.imageCalls;

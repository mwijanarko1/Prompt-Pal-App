...
const WinnerCard = memo(function WinnerCard({ user, type }: WinnerProps) {
	...
	return (
		<View
			className={`items-center ${isGold ? "-mt-8" : "mt-4"}`}
		...
		>
			<View className="relative mb-2">
				...
				>
					<Text className="text-[8px] font-black text-black uppercase tracking-widest">
						{label}
					</Text>
				</View>
			</View>
			...
			<Text className="text-primary text-[10px] font-black uppercase tracking-[2px] mt-1">
				{user.totalXp || user.points}
			</Text>
		</View>
	);
});
...
const RankItem = memo(function RankItem({
	...
	return (
		<Card className="mb-3 flex-row items-center p-5 rounded-[32px] border-0 bg-surfaceVariant/20">
			<Text className="text-onSurfaceVariant text-lg font-black w-10">
				{rank}
			</Text>
			...
			<View className="items-end">
				...
				<Text className="text-onSurfaceVariant text-[8px] font-black uppercase tracking-widest">
					PTS
				</Text>
			</View>
		</Card>
...
const CurrentUserCard = memo(function CurrentUserCard({
	currentUser,
	showFriendRank,
}: CurrentUserCardProps) {
	const rank =
		showFriendRank && currentUser.rank
			? currentUser.rank
		...
		<View className="absolute bottom-10 left-6 right-6">
			<Card className="bg-secondary p-6 rounded-full flex-row items-center border-0 shadow-2xl shadow-secondary/50">
				...
				<View className="w-12 h-12 rounded-full overflow-hidden mr-4 border-2 border-white/30">
					<Image
						source={{ uri: currentUser.avatarUrl || currentUser.avatar }}
						style={{ width: "100%", height: "100%" }}
						contentFit="cover"
					/>
				</View>
				<View className="flex-1">
					...
					<Text className="text-white/70 text-[8px] font-black uppercase tracking-[2px]">
						{rankLabel}
					</Text>
				</View>
				<View className="items-end">
					<Text className="text-white text-xl font-black">
						{currentUser.totalXp || currentUser.points || 0}
					</Text>
					<Text className="text-white/70 text-[8px] font-black uppercase tracking-widest">
						POINTS
					</Text>
				</View>
			</Card>
...
export default function RankingScreen() {
	...
	const [friendsCurrentUser, setFriendsCurrentUser] =
		useState<LeaderboardUser | null>(null);
	const [, setTotalFriends] = useState(0);
	...
	const topWinners =
		activeTab === "global" ? globalTopWinners : friendsTopWinners;
	const rankList = activeTab === "global" ? globalRankList : friendsRankList;
	const currentUser =
		activeTab === "global" ? globalCurrentUser : friendsCurrentUser;
	...
	useEffect(() => {
		const fetchGlobalLeaderboard = async () => {
			try {
				...
				const leaderboardData = await convexHttpClient.query(
				...
				if (user?.id) {
					const stats = await convexHttpClient.query(
						api.queries.getMyUserStatistics,
						{},
					);
					userRankData = {
						userId: user.id,
				...
				const top3 = leaderboardData.leaderboard
					.slice(0, 3)
					.map(toLeaderboardUser);
				const rest = leaderboardData.leaderboard
					.slice(3)
					.map(toLeaderboardUser);
				...
				if (userRankData && userRankData.globalRank <= 3) {
					const userIndex = top3.findIndex(
						(u: any) => u.globalRank === userRankData.globalRank,
					);
					if (userIndex >= 0) {
	...
	useEffect(() => {
		const fetchFriendsLeaderboard = async () => {
			try {
				...
				const friendsData: FriendsLeaderboardResponse =
					await convexHttpClient.query(api.queries.getFriendsLeaderboard, {
						limit: 50,
					});
				...
				const rankedTop3 = top3.map((friend, index) => ({
					...friend,
					rank: index + 1,
				}));
				const rankedRest = rest.map((friend, index) => ({
					...friend,
					rank: index + 4,
				}));
	...
	const FriendsEmptyState = memo(function FriendsEmptyState() {
		return (
			<View className="items-center py-16 px-8">
				...
				<Text className="text-onSurface text-xl font-black mb-2">
					No Friends Yet
				</Text>
	...
	return (
		...
		>
			...
			<View className="px-6 pt-2 pb-4 flex-row justify-between items-center">
				<View className="flex-1" />
				<Text className="text-onSurface text-xl font-black uppercase tracking-[2px]">
					Leaderboard
				</Text>
			...
			<View className="px-6 mb-8">
				<View className="flex-row bg-surfaceVariant/30 p-1.5 rounded-full border border-outline/10">
					...
					>
						<Text
							className={`text-[10px] font-black uppercase tracking-widest ${activeTab === "global" ? "text-onSurface" : "text-onSurfaceVariant"}`}
						>
							Global
						</Text>
					</Pressable>
					...
					>
						<Text
							className={`text-[10px] font-black uppercase tracking-widest ${activeTab === "friends" ? "text-onSurface" : "text-onSurfaceVariant"}`}
						>
							Friends
						</Text>
					</Pressable>
				...
				ListEmptyComponent={
					activeTab === "friends" ? (
						...
						<View className="items-center py-12">
							...
							<Text className="text-onSurfaceVariant text-lg font-bold mt-4">
								...
								<View className="mt-8 w-full px-6">
									<Card className="bg-secondary p-6 rounded-full flex-row items-center border-0 shadow-2xl shadow-secondary/50">
										...
										<View className="w-12 h-12 rounded-full overflow-hidden mr-4 border-2 border-white/30">
											<Image
												source={{
													uri: currentUser.avatarUrl || currentUser.avatar,
												}}
												style={{ width: "100%", height: "100%" }}
												contentFit="cover"
											/>
			...
			{rankList.length > 0 && currentUser && (
				<CurrentUserCard
					currentUser={currentUser}
					showFriendRank={activeTab === "friends"}
				/>
			)}
		</SafeAreaView>
	);

...
export function evaluatePractice1Prompt(prompt: string): PracticeEvaluation {
  ...
  const hasBehavior = containsAny(normalizedPrompt, [/\balert\b/, /\bon click\b/, /\bwhen clicked\b/, /\bshow\b.*\bhello\b/, /\bclick\b/]);
  const hasStyle = containsAny(normalizedPrompt, [/\borange\b/, /\brounded\b/, /\bround\b/, /\bwhite text\b/]);
  const matchesLabel = containsAny(normalizedPrompt, [/\bsay hello\b/, /\bhello\b/]);
  ...
  return {
    ...
    nudge: success ? undefined : `Add ${joinMissingParts(missingParts)} so the instruction is complete.`,
    takeaway:
...
export function evaluatePractice2Prompt(prompt: string): PracticeEvaluation {
  ...
  const hasInput = containsAny(normalizedPrompt, [/\binput\b/, /\btext field\b/, /\btextbox\b/, /\btype\b/]);
  const hasAddMechanism = containsAny(normalizedPrompt, [/\bbutton\b/, /\badd\b/, /\benter\b/, /\bclick\b/]);
  const taskAppearsInList = containsAny(normalizedPrompt, [/\bappend\b/, /\badd to (the )?list\b/, /\bshow up\b/, /\btask list\b/, /\bnew task\b/]);
  const inputClears = containsAny(normalizedPrompt, [/\bclear\b/, /\breset\b/, /\bempty\b.*\binput\b/]);
  ...
  const success = hasInput && hasAddMechanism && taskAppearsInList && score >= 4;
  const missingParts = [
  ...
  return {
    ...
    nudge: success ? undefined : `Spell out ${joinMissingParts(missingParts)} so the behavior is testable.`,
    takeaway:
...
<script>
  ...
  ${hasAddMechanism ? `document.getElementById('addBtn')?.addEventListener('click', () => {
    const value = input?.value?.trim();
    ${taskAppearsInList ? `if (value) {
...
export function evaluatePractice3Prompt(prompt: string): PracticeEvaluation {
  ...
  const togglesCompletedState = containsAny(normalizedPrompt, [/\bline-through\b/, /\bstrikethrough\b/, /\bcross(ed)? out\b/, /\btoggle\b.*\bcomplete\b/, /\bmark\b.*\bcomplete\b/]);
  const blocksEmptyInput = containsAny(normalizedPrompt, [/\bempty\b/, /\bblank\b/, /\btrim\b/, /\bdon't add\b/, /\bdo not add\b/, /\bprevent\b.*\bempty\b/]);
  const newTasksClickable = containsAny(normalizedPrompt, [/\bnew tasks?\b.*\bclick/, /\bnew items?\b.*\bclick/, /\bsame behavior\b/, /\bnew tasks?\b.*\bline-through\b/]);
  const keepsExistingUi = containsAny(normalizedPrompt, [/\bkeep\b.*\b(styling|style|layout|ui|design)\b/, /\bdon't change\b/, /\bdo not change\b/, /\bwithout changing anything else\b/, /\bas is\b/]);
  ...
  const missingParts = [
    ...
    !keepsExistingUi && "a guardrail to keep the existing layout and styling intact",
  ].filter(Boolean) as string[];
  ...
  return {
    ...
    nudge: success ? undefined : `Call out ${joinMissingParts(missingParts)} so the fix is precise.`,
    takeaway:
  ...
  const attachTaskBehavior = (item) => {
    ${togglesCompletedState ? `item.addEventListener('click', () => {

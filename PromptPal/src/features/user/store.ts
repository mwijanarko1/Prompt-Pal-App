			...
			syncWithBackend: async () => {
				try {
					...
					await convexHttpClient.mutation(
						api.mutations.updateUserStatistics,
						progressData,
					);
				} catch {
					// Sync is retried by background flows.
			...
			loadFromBackend: async (userId?: string) => {
				try {
						...
						const apiModule = apiModules?.find(
							(api: any) =>
								...
								api.category.toLowerCase() ===
									defaultModule.category.toLowerCase(),
						);
					...
					const quest = await convexHttpClient.mutation(
						api.mutations.getOrAssignCurrentQuest,
						{
							appId: "prompt-pal",
						},
					);
					...
					if (
						...
						console.warn(
							"Authentication failed when loading user data, attempting to refresh...",
						);
							...
							const [apiModules, progressRows] = await Promise.all([
								...
								userId
									? convexHttpClient.query(
											api.queries.getUserModuleProgress,
											{},
										)
									: Promise.resolve([]),
							]);

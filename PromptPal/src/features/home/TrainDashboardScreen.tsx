		...
		<View className="bg-info p-6 rounded-[32px] mb-10 overflow-hidden shadow-lg shadow-info/30">
			<View className="flex-row justify-between items-center mb-6">
				...
				<View className="flex-row items-center">
					...
					<Text
						className={`text-white text-xs font-black ml-1.5 uppercase tracking-tighter ${
							timeRemaining <= 0 ? "text-red-300" : ""
						}`}
					>
						{formatTimeRemaining(timeRemaining)}{" "}
						{timeRemaining > 0 ? "left" : ""}
					</Text>
				</View>
			</View>

			<Text className="text-white text-2xl font-black mb-2">{quest.title}</Text>
			<Text className="text-white/80 text-sm mb-8 font-medium leading-5">
				{quest.description}
			</Text>
			...
			<View className="flex-row justify-between items-center">
				...
				{quest.completed ? (
					...
					>
						<Text className="text-info font-black text-sm uppercase tracking-widest">
							Start Quest
						</Text>
					</Pressable>
				)}
...
const ModuleCard = memo(function ModuleCard({
	...
	return (
		<View
			className={`bg-surface border border-outline/30 rounded-[32px] mb-8 overflow-hidden shadow-sm ${isLocked ? "opacity-60" : ""}`}
		>
			{/* Header Pattern Area */}
			...
			<View className="p-6">
				<Text
					className={`text-2xl font-black mb-4 ${isLocked ? "text-onSurfaceVariant" : "text-onSurface"}`}
				>
					{title}
				</Text>
					...
					{isLocked ? (
						<>
							<Ionicons
								...
								style={{ marginRight: 8 }}
							/>
							<Text className="text-gray-500 font-black text-sm uppercase tracking-widest">
								Locked
							</Text>
						</>
					) : (
						<>
							<Text className="text-primary font-black text-sm uppercase tracking-widest mr-2">
								{buttonText}
							</Text>
							{buttonText === "Continue Learning" && (
...
export function TrainDashboardScreen() {
	...
	const backendCompletedIds =
		useQuery(api.queries.getCompletedLevelIds, { appId: "prompt-pal" }) ?? [];
	...
	const getModuleLevelInfo = useCallback(
		(moduleId: string) => {
			const type = MODULE_TYPE_MAPPING[moduleId];
			if (!type) return null;

			const moduleLevels = allLevels
				.filter((l) => l.type === type && !isDailyQuestLevelId(l.id))
			...
			const currentLevel =
				moduleLevels.find((l) => !effectiveCompletedLevels.includes(l.id)) ||
				moduleLevels[moduleLevels.length - 1];
	...
	const renderModuleItem = useCallback(
		({ item }: { item: LearningModule }) => {
			...
			const moduleLevels =
				allLevels?.filter(
					(l) => l.type === type && !isDailyQuestLevelId(l.id),
				) || [];
			const completedLevelsInModule = moduleLevels.filter((l) =>
				effectiveCompletedLevels.includes(l.id),
			).length;
			const actualProgress =
				moduleLevels.length > 0
					? Math.round((completedLevelsInModule / moduleLevels.length) * 100)
					: 0;
		...
		[allLevels, getModuleLevelInfo, effectiveCompletedLevels],
	);
		...
		>
			...
			<View className="px-6 pt-4 pb-4 flex-row justify-between items-center">
				<View className="flex-row items-center flex-1">
					...
					<View>
						<Text className="text-onSurfaceVariant text-[8px] font-black uppercase tracking-[3px] mb-0.5">
							{getGreeting()}
						</Text>
			...
			>
				<View className="px-6 mb-6">
					...
					<Text className="text-onSurfaceVariant text-sm mt-1 font-medium">
						Learn prompts, challenges, XP, streaks.
					</Text>
				</View>
				...
				<View className="px-6 mb-10">
					<View className="flex-row justify-between items-center mb-2.5">
						<Text className="text-onSurface text-[10px] font-black uppercase tracking-[2px]">
							Overall Mastery
						</Text>
						<Text className="text-onSurfaceVariant text-xs font-black">
							{overallProgress.current.toLocaleString()} /{" "}
							{overallProgress.total.toLocaleString()} XP
						</Text>
					</View>
					...
					<View className="px-1 pb-5">
						<View className="flex-row items-start justify-between mb-5">
							<View>
								...
								<Text className="text-onSurface text-[32px] font-black tracking-tight">
									Settings
								</Text>
						...
						<View className="rounded-[28px] bg-surfaceVariant/30 border border-outline/10 p-5">
							...
							<View className="mt-5 pt-5 border-t border-outline/10">
								<View className="flex-row items-center justify-between">
									<View>
										<Text className="text-onSurface text-xs font-black uppercase tracking-[2px]">
											Session
										</Text>
										<Text className="text-onSurfaceVariant text-sm mt-1">

			...
			<View className="w-full max-w-[520px] self-center min-w-0">
				...
				{sections.length > 0 ? (
					<Animated.View
						...
						className="mb-5 min-w-0 rounded-[20px] border border-outline/10 bg-surfaceVariant/15 p-4"
					>
									...
									<View className="min-w-0 flex-1 p-3">
										<View className="mb-1.5 flex-row items-center justify-between gap-3">
											<View className="min-w-0 flex-1 flex-row items-center">
												...
												<Text
													className={`min-w-0 flex-1 text-[10px] font-black uppercase tracking-[2px] ${tone.text} ${section.icon ? "ml-1.5" : ""}`}
												>
													{section.title}
												</Text>
											</View>
											{section.badge ? (
												<View
													className={`px-2.5 py-1 rounded-full ${tone.badge}`}
												>
													<Text
														className={`text-[10px] font-bold ${tone.text}`}
													>
														{section.badge}
													</Text>
												</View>
										...
										{section.code ? (
											<View className="min-w-0 bg-surface rounded-xl p-2.5 border border-outline/10 mt-2">
												<Text
													className="min-w-0 text-onSurface text-[11px] font-mono leading-4"
													selectable
												>
													{section.code}
												</Text>
					...
					<View className="rounded-[20px] border border-outline/15 bg-surface p-4">
						...
						{wordCountLabel ? (
							<View className="mb-3">
								<View className="flex-row justify-between items-center mb-1.5">
									...
									<Text
										className={`text-[11px] font-bold ${progressTextClass}`}
									>
										{wordCountLabel}
									</Text>
								</View>
								<View className="h-1.5 bg-surfaceVariant rounded-full overflow-hidden">
									<View
										...
										style={{
											width: `${Math.max(0, Math.min(wordProgress ?? 0, 100))}%`,
										}}
									/>
								</View>
							</View>
						...
						<View className="flex-row flex-wrap">
							<Badge
								...
								variant="surface"
								className="bg-surfaceVariant mr-2 mb-2 border-0 px-2.5"
							/>
							<Badge
				...
				{attemptView ? (
					...
					>
						...
						<Card
							className="w-full min-w-0 overflow-hidden rounded-[20px] border border-outline/10 p-0"
							variant="elevated"
						>

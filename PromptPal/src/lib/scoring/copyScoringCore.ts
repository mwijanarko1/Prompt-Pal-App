...
export function buildCopyAnalysisPrompt(text: string, brief: CopyBriefContext): string {
  const briefParts = [];
...
export function parseCopyMetrics(resultText: string, fallbackText: string, brief: CopyBriefContext): CopyMetric[] {
  try {
    ...
    return COPY_METRIC_LABELS.map(label => ({
      label,
      value: clamp(Number(parsed[label.replace(/ /g, '_').toUpperCase()] ?? parsed[label] ?? 50)),
    }));
  } catch {
...
export function calculateFallbackMetrics(text: string, brief: CopyBriefContext): CopyMetric[] {
  const scores: Record<string, number> = {};

  scores.TONE = brief.briefTone ? calculateToneScore(text, brief.briefTone) : 60;
  scores.PERSUASION = calculatePersuasionScore(text);
  scores.CLARITY = calculateClarityScore(text);
  scores['AUDIENCE FIT'] = brief.briefTarget ? calculateAudienceScore(text, brief.briefTarget) : 60;
...
export function checkRequiredElements(text: string, requiredElements?: string[]): CopyElementChecks {
...
export function isWithinWordLimit(wordCount: number, limits: { min?: number; max?: number }): boolean {
...
export function calculateCopyOverallScore(
  ...
  const avgMetricScore = metrics.reduce((sum, metric) => sum + metric.value, 0) / Math.max(metrics.length, 1);
...
export function generateCopyFeedback(input: {
  ...
  const { metrics, elementChecks, wordCount, limits, overallScore, passingScore, promptFeedback = [] } = input;
  const feedback = [...promptFeedback];

  if (passingScore && overallScore >= passingScore) {
    feedback.unshift('Strong result. Your prompt produced convincing copy that met the brief.');
    return dedupeFeedback(feedback);
  }

  const lowMetrics = metrics.filter(metric => metric.value < 60);
  if (lowMetrics.length > 0) {
    feedback.push(`Focus on improving ${lowMetrics.map(metric => metric.label.toLowerCase()).join(', ')}.`);
  }

  if (limits.min && wordCount < limits.min) {
    feedback.push('Ask for a slightly fuller draft so the model has room to develop the message.');
  } else if (limits.max && wordCount > limits.max) {
    feedback.push('Add brevity guidance so the model stays tighter and more focused.');
  }

  if (elementChecks.missing.length > 0) {
    feedback.push('Ask for more concrete benefits, differentiators, or CTA language.');
  }

  const highMetrics = metrics.filter(metric => metric.value >= 80);
  if (highMetrics.length > 0) {
    feedback.push(`Strong ${highMetrics.map(metric => metric.label.toLowerCase()).join(' and ')}.`);
  }

  if (overallScore < 40) {
    feedback.push('Major revision needed. Push the model with clearer strategic direction.');
  } else if (overallScore < 70) {
    feedback.push('Good foundation. Add sharper constraints and a stronger conversion goal.');
  }
...
function calculatePersuasionScore(text: string): number {
  ...
  const textLower = text.toLowerCase();
  const matchCount = persuasiveWords.filter(word => textLower.includes(word)).length;
...
function calculateEngagementScore(text: string): number {
  ...
  const textLower = text.toLowerCase();
  const matchCount = engagementSignals.filter(signal => textLower.includes(signal)).length;

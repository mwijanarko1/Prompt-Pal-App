...
const createSafeSandbox = () => {
	...
	const safeGlobals = {
		...
		Object: Object.freeze({
			...
			hasOwn: (target: unknown, property: PropertyKey) =>
...
const EXECUTOR_CACHE = new Map<
	string,
	{ runner: Function; lineOffset: number }
>();
...
const DISALLOWED_PATTERNS: { pattern: RegExp; message: string }[] = [
	...
	{
		pattern: /\bglobalThis\b|\bwindow\b|\bdocument\b|\bglobal\b/i,
		message: "Access to global objects is not allowed.",
	},
	{
		pattern: /\bfetch\b|\bXMLHttpRequest\b|\bWebSocket\b/i,
		message: "Network access is not allowed.",
	},
	{
		pattern: /\blocalStorage\b|\bsessionStorage\b/i,
		message: "Storage access is not allowed.",
	},
	{
		pattern: /\bprocess\b|\brequire\b|\bimport\s+\w+/i,
		message: "Module access is not allowed.",
	},
	{
		pattern: /\bconstructor\b|\bprototype\b/i,
	...
	{
		pattern: /\[\s*constructor\s*\]/i,
		message: "Constructor access is not allowed.",
	},
	{
		pattern: /\.call\s*\(|\.apply\s*\(/i,
		message: "Function call/apply is not allowed.",
	},
...
function getErrorLocation(
	...
	const match =
		stack.match(/<anonymous>:(\d+):(\d+)/) || stack.match(/:(\d+):(\d+)/);
	if (!match) return {};
	...
	static prepare(options: Omit<ExecutionOptions, "inputs">): PreparedExecution {
		...
		if (code.length > 50000) {
			const error = new Error(
				"Code exceeds maximum length of 50000 characters",
			);
			error.name = "ValidationError";
			throw error;
		}

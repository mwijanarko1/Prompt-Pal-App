...
export class SyncManager {
	...
	private static async loadOfflineQueue(): Promise<void> {
		try {
			...
			if (queueData) {
				...
				logger.info("SyncManager", "Loaded offline queue", {
	...
	private static async processOfflineQueue(): Promise<void> {
		...
		logger.info("SyncManager", "Processing offline queue", {
			count: this.offlineQueue.length,
		});
		...
		for (const item of this.offlineQueue) {
			try {
				if (!this.isValidGameState(item.gameState)) {
					logger.warn("SyncManager", "Invalid game state in queue, dropping", {
						id: item.id,
					});
				...
				if (item.retries >= MAX_SYNC_RETRIES) {
					logger.warn("SyncManager", "Dropping item after max retries", {
						id: item.id,
					});
					processedIds.push(item.id);
				}
	 ...
	 */
	private static async addToOfflineQueue(
		gameState: Partial<GameState>,
	): Promise<void> {
	...
	private static async updateLastSyncTimestamp(): Promise<void> {
		try {
			...
			logger.error("SyncManager", error, {
				operation: "updateLastSyncTimestamp",
			});
		}
	}
	...
	private static async performSync(
		...
		try {
			...
			if (retryCount < MAX_SYNC_RETRIES) {
				...
				logger.warn(
					"SyncManager",
					`Sync failed, retrying (${retryCount + 1}/${MAX_SYNC_RETRIES})`,
					{ delay: backoffDelay },
				);
	...
	private static async fetchServerProgress(): Promise<Partial<GameState> | null> {
		try {
			const gameState = await convexHttpClient.query(
				api.queries.getUserGameState,
				{
					appId: "prompt-pal",
				},
			);
	...
	private static resolveConflicts(
		...
		switch (this.conflictResolution.strategy) {
			...
			case "server-wins":
				return {
					unlockedLevels:
						serverState.unlockedLevels || localState.unlockedLevels,
					completedLevels:
						serverState.completedLevels || localState.completedLevels,
				};
	...
	static async getSyncStatus(): Promise<SyncStatus> {
		...
		try {
			...
			logger.warn("SyncManager", "Failed to read last sync timestamp", {
				error,
			});
		}
	...
	static setConflictResolutionStrategy(
		...
		logger.info("SyncManager", "Conflict resolution strategy updated", {
			strategy,
		});
	}

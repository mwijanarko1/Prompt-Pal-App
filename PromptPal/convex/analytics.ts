...
export const getUserEngagementMetrics = query({
	...
	handler: async (ctx, args) => {
		...
		const totalSessions = activeUsers.filter(
			(event) => event.eventType === "session_start",
		).length;

		const levelCompletions = activeUsers.filter(
			(event) => event.eventType === "level_completed",
		);
		...
		const avgSessionLength =
			totalSessions > 0
				? activeUsers
						.filter((event) => event.eventType === "session_end")
						.reduce(
							(sum, event) =>
								sum +
								(event.timestamp -
									(event.eventData?.sessionStart || event.timestamp)),
							0,
						) / totalSessions
				: 0;
...
export const getLevelPerformanceAnalytics = query({
	...
	handler: async (ctx, args) => {
		...
		if (levelId) {
			sessionQuery = sessionQuery.filter((q) =>
				q.eq(q.field("levelId"), levelId),
			);
		}
		...
		return {
			...
			averageTimeSpent: Math.round(
				totalTime /
					Math.max(1, sessions.filter((s) => s.endedAt).length) /
					1000,
			), // seconds
...
export const getUserRetentionAnalytics = query({
	...
	handler: async (ctx, args) => {
		...
		const activeUserIds = new Set(
			activeUserEvents.map((event) => event.userId),
		);
		...
		return {
			...
			retentionRates,
			averageRetention:
				retentionRates.reduce((sum, r) => sum + r.retentionRate, 0) /
...
export const getAIUsageAnalytics = query({
	...
	handler: async (ctx, args) => {
		...
		const totalResponseTime = generations.reduce(
	...
	handler: async (ctx, args) => {
		...
		for (const [type, typeMetrics] of Object.entries(metricsByType)) {
			...
			aggregates[type] = {
				...
				min: Math.min(...values),
				max: Math.max(...values),
				p95: values.sort((a: number, b: number) => a - b)[
